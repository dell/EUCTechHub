# Returns value for Docking Firmware
# Return Type: String
...
<#
.Synopsis
   This PowerShell is using Dell Command | Monitor to collect values from DCIM_Chassis Class. Select Value for Docking Firmware.
...
if (($Check_Chassis_Type -eq 10) -or ($Check_Chassis_Type -eq 31) -or ($Check_Chassis_Type -eq 32))
    ...
    Foreach($i in $Dock_data)
        ...
        if($Check_CreationClassName[$DeviceCounter] -eq "DCIM_DockingStation")
            ...
            $OutputStatement = $Dock_Firmware_Value[$DeviceCounter]
            }
