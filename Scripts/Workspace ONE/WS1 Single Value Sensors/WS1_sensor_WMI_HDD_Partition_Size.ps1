# Returns value for HDD Partition Size
# Return Type: String
...
<#
.Synopsis
   This PowerShell is using WMI to collect values from Win32_DiskPartition. Select Value for HDD Partition Size
...
$Partition_Size_Value = $Partition_Data.Size
...
foreach($i in $Partition_Data)
    ...
    $Partition = $Partition+$Partition_Name_Value[$IndexCounter]+" "+$Partition_Size+[math]::Round($Partition_Size_Value[$IndexCounter]/1GB)+" GB "
