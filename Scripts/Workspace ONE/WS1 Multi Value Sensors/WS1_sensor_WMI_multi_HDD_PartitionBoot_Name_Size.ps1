# Returns value for HDD Partition Name, BootPartition, Size
# Return Type: String
...
foreach($i in $Partition_Data)
    ...
    $Partition = $Partition+$Partition_Name+$Partition_Name_Value[$IndexCounter]+" "+$Partition_Boot+$Partition_Boot_Value[$IndexCounter]+" "+$Partition_Size+[math]::Round($Partition_Size_Value[$IndexCounter]/1GB)+" GB "
